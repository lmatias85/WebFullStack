...
/* Función que permite agregar instrucciones en el arreglo */
function agregarInstruccion(strInstruccion) {
  instrucciones.push(strInstruccion);
}

/* COMPLETAR: Crear función que agregue la última dirección al arreglo de movimientos
y utilice actualizarUltimoMovimiento para mostrarlo en pantalla */
function agregarUltimoMovimiento(ultimoMovimiento,tipo) {
  var arrUltimos5 = [];
  var largoArr;
  movimientos.push(ultimoMovimiento);
  if (tipo === 'USUARIO'){
    movimientosUsuario.push(ultimoMovimiento);
    largoArr = movimientosUsuario.length;
    for (var i = 0; i <= 5; i++){
      arrUltimos5[i] = movimientosUsuario[largoArr];
      largoArr--;
    }
    actualizarUltimos5Movimientos(arrUltimos5);
  }
...
function mostrarInstrucciones(instrucciones) {
  agregarInstruccion("Utilizar las flechas para mover las fichas");
  agregarInstruccion("Ordernar las piezas hasta alcanzar la imagen objetivo");
...
function mostrarCartelGanador() {
  var mensajeMovimientos;
  var mensajeGanador;
  if(movimientos.length > 0)
    if(chequearSiGano()) {
      if (movimientosUsuario.length === 1)
        mensajeMovimientos = movimientosUsuario.length + " movimiento.";
      else
        mensajeMovimientos = movimientosUsuario.length + " movimientos.";
      mensajeGanador = document.getElementById('mensajeGanador');
      mensajeGanador.textContent += mensajeMovimientos;
      window.location.href = document.getElementById("ganador").href;
    }
}
...
function intercambiarPosicionesGrilla(filaPos1, columnaPos1, filaPos2, columnaPos2) {
  var auxPos1;
  auxPos1 = grilla[filaPos1][columnaPos1];
  grilla[filaPos1][columnaPos1] = grilla[filaPos2][columnaPos2];
  grilla[filaPos2][columnaPos2] = auxPos1;
}
...
function moverEnDireccion(direccion,tipo) {
  ...
  if (posicionValida(nuevaFilaPiezaVacia, nuevaColumnaPiezaVacia)) {
    ...
    agregarUltimoMovimiento(direccion,tipo);
  }
}
...
function actualizarUltimos5Movimientos(arrMovimientos){

  ultimos5Mov = document.getElementById('flecha');
  ultimos5Mov.textContent = '';

  for(var i= 0; i < arrMovimientos.length; i++){
    switch (arrMovimientos[i]) {
      case codigosDireccion.ARRIBA:
        ultimos5Mov.textContent += '↑';
        break;
      case codigosDireccion.ABAJO:
...
function mezclarPiezas(veces) {
  ...
  moverEnDireccion(direccion,'RANDOM');
